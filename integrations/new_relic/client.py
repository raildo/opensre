...
class NewRelicClient:
    ...
    def query_incidents(
        ...
        *,
        ...
        """Run the ``NrAiIncident`` query backing the alerts tool (plan.md §2).

        Returns the raw, unpaired transition rows — pairing rows by
        ``incidentId`` into one record per incident, decoding HTML entities,
        and flagging ``muted`` is the alerts tool's job (Phase 3), not the
        client's, so the same rows can also be inspected untouched in tests.
        """
